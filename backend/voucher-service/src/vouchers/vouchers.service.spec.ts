...
const mockVoucherActive: Voucher = {
  ...
  total_uses_left: 100, // Added total_uses_left
...
const mockVoucherUpcoming: Voucher = {
  ...
  total_uses_left: 50, // Added total_uses_left
  condition_type: VoucherConditionType.NONE,
...
const mockVoucherExpired: Voucher = {
  ...
  total_uses_left: 200, // Added total_uses_left
...
//   id: 10,
//   VoucherId: mockVoucherActive.id,
//   OwnerId: mockUserId,
//   ExpDate: mockVoucherActive.ends_at,
//   UsingTimeLeft: 0,
//   CreatedAt: new Date(),
//   UpdatedAt: new Date(),
//   voucher: mockVoucherActive,
// };
// const mockUserVoucherClaimedUpcoming: UserVoucher = {
//   id: 11,
//   VoucherId: mockVoucherUpcoming.id,
//   OwnerId: mockUserId,
//   ExpDate: mockVoucherUpcoming.ends_at,
//   UsingTimeLeft: 1,
//   CreatedAt: new Date(),
//   UpdatedAt: new Date(),
//   voucher: mockVoucherUpcoming,
// };
// const mockVoucherHistoryEntry: VoucherHistory = {
//   id: 20,
//   VoucherID: mockVoucherExpired.id,
//   UserID: mockUserId,
//   UseDate: new Date(yesterday.getTime() - 86400000), // Used one day before expiry
//   CreatedAt: new Date(),
//   UpdatedAt: new Date(),
// };
...
describe('VouchersService', () => {
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: getRepositoryToken(SellerVoucher),
          useValue: createMockRepository<SellerVoucher>(),
        },
      ],
    }).compile();
    ...
    // Object.values(voucherHistoryRepository).forEach((mockFn) =>
    //   mockFn.mockReset(),
    // );
    // Object.values(userVoucherRepository).forEach((mockFn) =>
    //   mockFn.mockReset(),
    // );
  });
  ...
  describe('create', () => {
    ...
    it('should successfully create a voucher (and uppercase code)', async () => {
      ...
      repository.create.mockImplementation(
        (dto) =>
          ({ ...dto, total_uses_left: dto.total_usage_limit }) as Voucher,
      ); // Simulate create and init total_uses_left
      ...
      expect(repository.create).toHaveBeenCalledWith(
        expect.objectContaining({ ...createDto, code: 'NEW15' }),
      );
  ...
  describe('findAll', () => {
    ...
    it('should filter by status: active', async () => {
      ...
      const expectedOptions: FindManyOptions<Voucher> = {
        ...
        order: { created_at: 'DESC' },
        where: {
          starts_at: LessThanOrEqual(expect.any(Date)),
          ends_at: MoreThanOrEqual(expect.any(Date)),
        },
      };
      const result = await service.findAll(queryDto);
      expect(repository.findAndCount).toHaveBeenCalledWith(expectedOptions);
    ...
    it('should combine search and status filters', async () => {
      ...
      const queryDto: QueryVoucherDto = {
        search: searchTerm,
        status: VoucherStatusQuery.UPCOMING,
      };
      repository.findAndCount.mockResolvedValue([[mockVoucherUpcoming], 1]);
      const expectedOptions: FindManyOptions<Voucher> = {
        skip: 0,
        ...
        where: [
          {
            name: ILike(`%${searchTerm}%`),
            starts_at: MoreThan(expect.any(Date)),
          },
          {
            code: ILike(`%${searchTerm}%`),
            starts_at: MoreThan(expect.any(Date)),
  ...
  describe('update', () => {
    ...
    it('should throw NotFoundException if voucher to update is not found', async () => {
      repository.findOneBy.mockResolvedValue(null);
      await expect(service.update(voucherId, updateDto)).rejects.toThrow(
        NotFoundException,
      );
      expect(repository.findOneBy).toHaveBeenCalledWith({ id: voucherId });
    });
    ...
    it('should throw BadRequestException if updated code already exists for another voucher', async () => {
      ...
      await expect(service.update(voucherId, updateCodeDto)).rejects.toThrow(
        BadRequestException,
      );
  ...
  describe('remove', () => {
    ...
    it('should throw NotFoundException if voucher to remove is not found', async () => {
      ...
      await expect(service.remove(voucherId)).rejects.toThrow(
        NotFoundException,
      );
      expect(repository.delete).toHaveBeenCalledWith(voucherId);
    });
