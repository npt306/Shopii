  ...
  const fetchCategories = async () => {
    try {
      const response = await axios.get(
        `${EnvValue.API_GATEWAY_URL}/api/categories/names`
      );
      ...
      result = result.filter(
        (product) =>
          ...
          product.details.some(
            (detail) =>
              (detail.type_1 &&
                detail.type_1.toLowerCase().includes(searchLower)) ||
              (detail.type_2 &&
                detail.type_2.toLowerCase().includes(searchLower))
          )
      );
    }
  ...
  return (
    <div className="p-2 pt-4">
        ...
        <div className="flex">
          {types.map((type, index) => (
            <button
              ...
              className={`px-4 py-2 relative bg-gray-100 ${
            ...
            <div className="flex items-center space-x-2 mb-2">
              ...
              <button
                ...
                onClick={() => {
                  ...
                  result = result.filter((product) =>
                    product.details.some(
                      (detail) => detail.quantity > 0 && detail.quantity <= 20
                    )
                  );
                  setFilteredProducts(result);
                }}
        ...
        {currentParam === "need_optimized" && (
          <>
            ...
            <div className="flex items-center space-x-2 mb-4">
              <span className="text-gray-700 text-sm font-medium">Vấn đề</span>
              <button
                className={`px-3 py-1 border rounded bg-white ${
                  selectedIssue === "fail"
                    ? "border-orange-500 text-orange-500"
      ...
      <div className="m-3 bg-white p-4 rounded shadow">
        ...
        {currentType === "live" && currentParam === "all" && (
          <AllActived products={filteredProducts} />
        )}
        {currentType === "live" && currentParam === "restock" && (
          <Restock products={filteredProducts} />
        )}
        {currentType === "live" && currentParam === "need_optimized" && (
          <Need_optimized />
        )}
        {currentType === "violation" && currentParam === "banned" && <Banned />}
        {currentType === "violation" && currentParam === "deboosted" && (
          <Deboosted />
        )}
        {currentType === "violation" && currentParam === "deleted" && (
          <Deleted />
        )}
        {currentType === "reviewing" && <Reviewing />}
        {currentType === "unpublished" && currentParam === "unlisted" && (
          <Unlisted />
        )}
