...
export const userService = {
    login: async (username: string, password: string): Promise<LoginResponse> => {
        try {
            const response = await axios.post<LoginResponse>('http://localhost:3000/Users/login', { username, password });
    ...
    register: async (userData: UserDto): Promise<RegisterResponse> => {
        try {
            const response = await axios.post<RegisterResponse>('http://localhost:3000/Users/login', userData);
            return response.data;
        } catch (error) {
            const axiosError = error as AxiosError<{ message: string }>;
    ...
    verifyEmail: async (email: string): Promise<{ success: boolean; message: string }> => {
        try {
            const response = await axios.post<{ success: boolean; message: string }>('http://localhost:3000/Users/send-verification-email', { email });
            return response.data;
        } catch (error) {
            const axiosError = error as AxiosError<{ message: string }>;
